...
def learn_route():
    ...
    if request.method == 'POST':
        options = generic_learn_route(
            request.form['op'], request.form, request.files['file'] if 'file' in request.files else None)
    try:
        # Retrieve knowledge.
...
allowed_endpoints = ['add_image', 'delete_image',
                     'add_text', 'add_url', 'delete_text', 'query']
...
def api_learn_add_del_route(op):
    if not op in allowed_endpoints:
        abort(404)
    session['username'] = database.get_username(
        request.form['interface'], request.form['username'])
    ...
    if not op == 'query':
        options = generic_learn_route(
            op, request.form, request.files['file'] if 'file' in request.files else None)
    else:
        try:
            # Retrieve knowledge.

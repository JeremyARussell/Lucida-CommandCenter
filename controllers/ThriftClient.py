...
class ThriftClient(object):
    ...
    def executeThreadServiceRequest(self, service_name, inputData, LUCID, threadIDValue):
        print("Thread ", threadIDValue, "executing",
              service_name, "with input", inputData)
        service = self.SERVICES[service_name]
        host, port = service.get_host_port()
        tag_list = [host, str(port)]
        query_input_list = [self.create_query_input(
            service.input_type, inputData, tag_list)]
...
# TODO: split function into separate functions (DCM, creating QuerySpec)
    def infer(self, LUCID, workflow_name, text_data, image_data):
        ...
        while not workflow.isEnd:
            ...
            i = 0
            for x in resultText:
                # Text information must be unicode'd and array'd to be properly passed. IMAGE DATA DOES NOT HAVE THIS DONE TO IT.
                resultText[i] = [unicode(resultText)]
                i += 1
            ...
            for x in workflow.batchedData:
                print("_____Thread" + str(threadID) + "," +
                      str(x.serviceName) + "," + str(x.argumentData))
